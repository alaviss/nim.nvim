...
function! s:OnEnd() dict
  ...
  if data.newId != -1
    if data.srcId != -1
      call nvim_buf_clear_highlight(data.buffer, data.srcId, 0, -1)
    endif
    let data.srcId = data.newId
    let data.newId = -1
    if data.queued
      let data.queued = v:false
      call s:HighlightBuffer(data)
    endif
  endif
endfunction
...
function! s:HighlightBuffer(hldata) abort
  if a:hldata.newId != -1
  ...
  let opts = {'onReply': function('s:OnReply'),
      ...
      \       'hldata': a:hldata}
  call nim#suggest#utils#Query(a:hldata.buffer, -1, -1, 'highlight', opts, v:true)
endfunction

function! nim#suggest#highlight#HighlightBuffer()
  call nim#suggest#highlight#InitBuffer()
  call s:HighlightBuffer(b:nimSugHighlight)
endfunction
